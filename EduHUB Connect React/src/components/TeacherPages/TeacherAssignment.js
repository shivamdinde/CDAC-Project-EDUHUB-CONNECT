...
function TeacherAssignment() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      // Send a POST request with Axios to save assignment
      await axios.post("http://localhost:9090/api/assignment/give", formData, {
        headers: {
      ...
      console.error(error);
      alert("Failed to give assignment.");
    }
  };
          ...
          <MDBCard className="my-5 bg-glass">
            <MDBCardBody>
              <div className="d-flex flex-column justify-content-center h-custom-2 w-75 pt-4">
                <h3 className="Auth-form-title">Upload Assignment</h3>
                <MDBInput
                ...
                <div className="mb-4 mx-5 w-100">
                  <label htmlFor="fileInput" className="form-label">Upload Assignment File</label>
                  <input
